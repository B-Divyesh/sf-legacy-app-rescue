...
The public `v0.1.2` Linux archive passed its published `SHA256SUMS` entry. The live `/install.sh` installed it into an empty temporary directory only after checksum verification; the installed binary reported `rescue 0.1.2` and completed `--json demo`. The public `.deb` metadata is `0.1.2-1`. The public RPM also passed its `SHA256SUMS` entry and reports `legacy-app-rescue 0:0.1.2-1 x86_64` from authoritative RPM headers. `npm run verify:package-managers` confirmed the Homebrew, Scoop, and winget manifests resolve to this repaired release.
